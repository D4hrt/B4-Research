...
リポジトリには、設計検討・拡張可能性の検証として実装した試行的なコンポーネント（`tinytroupe/agent/human_*.py`, `tinytroupe/agent/learnable_*.py`, `refine_persona.py`, `evaluate_refined.py`, `try/` など）も含まれているが、いずれも本研究の評価・報告対象には含めていない。
