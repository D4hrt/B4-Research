    ...
    def add_documents(self, new_documents) -> list:
        ...
        if len(new_documents) > 0:
            ...
            for document in new_documents:
                ...
                # Pydanticの`text`属性は読み取り専用なので、新しいDocumentを作成
                sanitized_document = Document(
                    text=sanitized_text,
                    metadata=document.metadata.copy() if hasattr(document, 'metadata') else {},
                    id_=document.id_ if hasattr(document, 'id_') else None
                )
                
                logger.debug(f"Document text after sanitization: {sanitized_document.text}")
                # ========== ここまで変更 ==========

                # add the new document to the list of documents after all sanitization and checks
                # ========== 変更: documentをsanitized_documentに変更 ==========
                self.documents.append(sanitized_document)

                if sanitized_document.metadata.get("semantic_memory_id") is not None:
                    # if the document has a semantic memory ID, we use it as the identifier
                    name = sanitized_document.metadata["semantic_memory_id"]
                    ...
                    if name in self.name_to_document:
                        # ========== 変更: documentをsanitized_documentに変更 ==========
                        self.name_to_document[name].append(sanitized_document)
                    else:
                        # ========== 変更: documentをsanitized_documentに変更 ==========
                        self.name_to_document[name] = [sanitized_document]
